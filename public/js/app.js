const App = {
    ...
    if (typeof web3 === 'undefined') {
      ...
      async function checkNetwork() {
        ...
        if (
          ...
          document.getElementById('incorrect-network').style.display =
            'initial';
  ...
  bindEvents: function () {
    ...
    createTokenForm.addEventListener('submit', async (evt) => {
      ...
      const tokenName = createTokenForm.querySelector(
        '#createToken_tokenName'
      ).value;
      const tokenSymbol = createTokenForm.querySelector(
        '#createToken_tokenSymbol'
      ).value;
      const imageCID = createTokenForm.querySelector(
        '#createToken_documentUri'
      ).value;
      const decimals = createTokenForm.querySelector(
        '#createToken_decimals'
      ).value;
      ...
      const tokenImage = `https://gateway.pinata.cloud/ipfs/${imageCID}`;
      const documentUri = `ipfs://${imageCID}`;
      const documentHash =
      ...
      manageTokenForm.querySelector('#manageToken_contractAddress').value =
        contract.address;
    ...
    manageTokenForm.addEventListener('submit', async (evt) => {
      ...
      const contractAddress = manageTokenForm.querySelector(
        '#manageToken_contractAddress'
      ).value;
  ...
  reloadManageToken: async function (contractAddress) {
...

    {
      ...
      createManager('transferOwnershipForm', template, async (evt) => {
        ...
        that.toggleFormDisabled(true);
        const address = transferOwnershipForm.querySelector(
          '#transferOwnership_address'
        ).value;
        try {
...

    {
      const template = `
                ...
                ${
                  isOwner
                    ? '<button type="submit">Renounce Ownership</button>'
                    : 'You are not owner'
                }
            `;
  ...
  requestWalletToTrackAsset: async function (
    contractAddress,
    tokenSymbol,
    decimals,
    tokenImage
  ...
  forceWatchAsset: async function (
    contractAddress,
    tokenSymbol,
    decimals,
    tokenImage
    ...
    while (
      !(await this.requestWalletToTrackAsset(
        contractAddress,
        tokenSymbol,
        decimals,
        tokenImage
